...
<div id="contenido">         
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group form-inline">
                    <label>Puntaje inicial</label><a href="#" data-toggle="tooltip" data-placement="right" title = "Debe indicar el puntaje inicial">
                    <i class="fa fa-question-circle" aria-hidden="true"></i></a><br>
            ...
            <div class="col-md-3">
                <div class="form-group form-inline">
                    <label>Puntaje final</label><a href="#" data-toggle="tooltip" data-placement="right" title = "Debe indicar el puntaje final">
                    <i class="fa fa-question-circle" aria-hidden="true"></i></a><br>
  ...
  <script>
    function limpiarIdioma() {
       $('#cboIdioma').val("").trigger('change');
    }

    function limpiarTipoCertificacion() {
        $('#cboTipoCertificacion').val("").trigger('change');
    }
    ...
    function buscarHojasVida() {
        $('#formHV').hide();
        $('#divDescargar').hide();
        if($('#cboIdioma').val() == "" && $('#cboTipoCertificacion').val() == "" && $('#puntajeInicial').val() == "" && $('#puntajeFinal').val() == "") {
        ...
        formData.append("idioma", $('#cboIdioma').val());
        formData.append("tipoCertificacion", $('#cboTipoCertificacion').val());
        formData.append("puntajeInicial", $('#puntajeInicial').val());
        formData.append("puntajeFinal", $('#puntajeFinal').val());
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/hojasVida/consultarHojasVidaIdioma",
            data: formData,
            processData: false,
            contentType: false,
                ...
                if (response !== "") {
                    ...
                    for (var i = 0; i < hojasVida.length; i++) {
                        tblHojasVida.row.add([
                            ...
                            hojasVida[i].idioma,
                            hojasVida[i].tipoCertificacion,
                            hojasVida[i].puntaje,
                            hojasVida[i].validado,
    ...
    function descargar() {
        ...
        $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/hojasVida/descargarHojasVidaIdioma?idioma=" + $('#cboIdioma').val() + "&tipoCertificacion=" + $('#cboTipoCertificacion').val() + "&puntajeInicial=" + $('#puntajeInicial').val() + "&puntajeFinal=" + $('#puntajeFinal').val(),
                processData: false,
                contentType: false,
                success: function (response) {
                    if (response != "") {
